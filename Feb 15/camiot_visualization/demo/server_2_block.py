...
from finger_control import finger_control_f,finger_control_f1

strbroker = "192.168.1.26"
tServer = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
tServer.bind(('192.168.1.24', 8000))
tServer.listen(0)
    ...
    if msg.topic == 'image': 
        ...
        if info == 'rec':
            ...
            if control_signal == 'Down':
                ...
                if (x_mid > width*3.2//4) or (direction > 0 and direction < np.pi/2 - size):
                    print('block 4')
                    block = 8
                    pyautogui.press('8')
                elif (x_mid < width*6//20) or(direction < 0 and direction > - np.pi/2 + size):
                    print('block 1')
                    block = 2
                    pyautogui.press('2')
